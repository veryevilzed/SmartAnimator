...
export default class Parallel {
  constructor(params = {}, obj = undefined) {
    ...
    this.params.animators = _.map(this.params.animators, anim => {
      if (anim instanceof Animator || anim.params) {return anim;}
  ...
  update(df) {
    ...
    if (this.params.onUpdate) {this.params.onUpdate(this);}
  ...
  stop(applyTo = true) {
    this.run = false;
    if (applyTo) {_.forEach(this.params.animators, a => a.stop(applyTo));}
    return this;
  }
}
