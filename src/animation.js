...
export default class Animation {
  ...
  stop(applyTo = true) {
    this.run = false;
    if (applyTo) {this.__apply(this.params.to);}
    return this;
  }
  ...
  update(dt) {
    ...
    if (_dt > this.params.time) {_dt = this.params.time;}
    _.forEach(this._from, (value, key) => {
      if (typeof this.params.to[key] === 'number') {
        if (this.params.to[key] !== undefined) {
          _.set(this.obj, key, this.params.easing(_dt, value, this._to[key] - value, this.params.time));
        }
    ...
    _.forEach(this.params.strings, (value, key) => {
      if (typeof value === 'string') {_.set(this.obj, key, value); }
      if (typeof value === 'function') {_.set(this.obj, key, value(this.obj));}
    });
